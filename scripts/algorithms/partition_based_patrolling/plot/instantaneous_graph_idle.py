...
for idx,no_agents in enumerate(no_agents_list):
    ...
    for m,algo_name in enumerate(algo_list):
        ...
        fig.add_trace(go.Scatter(x=stamps, y=graph_idle,mode='lines',marker=dict(color=color_list[m]),legendgroup=m+1,name=algo_name,showlegend=(True if idx==0 else False)),row=int(idx/col_size)+1,col=idx%col_size+1)
