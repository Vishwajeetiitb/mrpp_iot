...
def select_random_points_on_line_segments(line_segments, n):
    total_length = sum(((x2-x1)**2 + (y2-y1)**2)**0.5 for ((x1, y1), (x2, y2)) in line_segments)
    selected_points = []
    for i in range(n):
...
random_points = select_random_points_on_line_segments(line_segments, 10)
# Plot 
