...
for dir in all_dir:
    if os.path.exists(dir + '/agents_masterdata.npy'):
        ...
        agent_masterdata = np.concatenate((agent_masterdata,np.array(tmp_agent_masterdata)),axis=0)
        sort_indices = stamps.argsort()
        stamps_final = np.take(stamps,sort_indices)
        data_arr_final=np.take(data_arr,sort_indices,axis=0)
        agent_masterdata_final = np.take(agent_masterdata,sort_indices,axis=0)
